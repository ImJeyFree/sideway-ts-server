...
int32_t STC_CALL stc_command(StcHandle h,const char* text){if(!h||!text)return STC_INVALID_ARGUMENT;try{auto op=nlohmann::json::parse(text).at("op").get<std::string>();return op=="load"||op=="cancel"||op=="shutdown"||op=="stop"?STC_OK:STC_FAILED;}catch(...){return STC_FAILED;}}
